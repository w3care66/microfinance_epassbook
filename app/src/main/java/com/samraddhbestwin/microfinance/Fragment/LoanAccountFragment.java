...
public class LoanAccountFragment extends Fragment implements LoanAdapter.LoanListener {
    ProgressBar progressBar;
    RecyclerView loanAccountRecyclerView;
    TextView noDataFoundTextView;
    LoanAdapter adapter;
    ArrayList<LoanListResponse.LoanListData> loanAccountList =new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_loan_account, container, false);
        ((HomeActivity)getActivity()).set_action_title(getString(R.string.loan_account_));
        ((HomeActivity)getActivity()).showNotificationIcon(true);
        progressBar = (ProgressBar)rootView.findViewById(R.id.spin_kit);
        loanAccountRecyclerView = (RecyclerView) rootView.findViewById(R.id.recyclerview_loan_account);
        noDataFoundTextView = (TextView) rootView.findViewById(R.id.tv_no_data_found);
        loanAccountRecyclerView.setLayoutManager( new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false));
        loanAccountRecyclerView.hasFixedSize();
        adapter = new LoanAdapter(this.getContext(), loanAccountList, "Loan", LoanAccountFragment.this);
        loanAccountRecyclerView.setAdapter(adapter);
                        ...
                        if (response.body().getCode() == 200){
                            if (response.body().getData().size() > 0){
                                for (int i=0; i< response.body().getData().size(); i++){
                                    ...
                                    loanAccountList.add(loanListData);
                                }
                                noDataFoundTextView.setVisibility(View.GONE);
                                loanAccountRecyclerView.setVisibility(View.VISIBLE);
                            }else {
                                noDataFoundTextView.setVisibility(View.VISIBLE);
                                loanAccountRecyclerView.setVisibility(View.GONE);
                            }
